...
- `GET /api/events` — Server-Sent Events stream for live UI refresh. One `ProjectWatcher` per project (shared across subscribers) polls two signatures every 1.5 s. Emits `log` when `.review-log.json` `mtime+size` changes and `worktree` when the git state changes (composite signature: `.git/HEAD` + `.git/index` mtimes, `git status --porcelain` output, and mtimes of every file that status lists — catches commits, staging, branch switches, and repeated edits to an already-modified file). The client handles them differently: `log` refetches comments + tool calls; `worktree` only refetches the file list + the currently-open file's content (Browse / Git-diff modes only). First subscriber starts the watcher; last one stops it.
...
| `src/watcher.ts` | `startProjectWatcher(dir, onEvent)` — 1.5 s interval comparing two signatures and invoking `onEvent({ type: 'log' \| 'worktree' })` on either change. `log` tracks `.review-log.json` `mtime+size`. `worktree` tracks `.git/HEAD` mtime, `.git/index` mtime, `git status --porcelain=v1 --untracked-files=all` output, and — folded in — the mtimes of every file that status lists (so repeated worktree edits to an already-modified file still fire an event). Returns a stop function. |
